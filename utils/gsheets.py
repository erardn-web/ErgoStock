...
CATEGORIES = [
    ...
    "Environnement", "Soins personnels", "Jeu / Loisir", "Évaluation / Bilan", "Formation / Documentation", "Autre",
]
...
def encode_disponibilites(tester: bool, preter: bool, donner: bool, vendre: bool) -> str:
    parts = []
    if tester: parts.append("À tester")
    if preter: parts.append("À prêter")
    if donner: parts.append("À donner")
    if vendre: parts.append("À vendre")
    return ", ".join(parts)


def decode_disponibilites(value: str) -> dict:
    v = value or ""
    return {
        "tester": "À tester" in v,
        "preter": "À prêter" in v,
        "donner": "À donner" in v,
