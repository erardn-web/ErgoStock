...
from utils.gsheets import (
    add_materiel, upload_photo_to_drive, CATEGORIES, ETATS,
...
# Checkboxes HORS formulaire
st.markdown("**🏷️ Disponibilités**")
dc1, dc2, dc3 = st.columns(3)
with dc1:
    dispo_tester = st.checkbox("🔬 À tester", key="new_tester")
with dc2:
    dispo_preter = st.checkbox("🤝 À prêter", key="new_preter")
with dc3:
    dispo_vendre = st.checkbox("💶 À vendre", key="new_vendre")
        ...
        with st.spinner("Enregistrement…"):
            mat_id = add_materiel({
                ...
                "Disponibilités":   encode_disponibilites(
                    st.session_state.get("new_tester", False),
                    st.session_state.get("new_preter", False),
                    st.session_state.get("new_vendre", False),
                ),
            })
