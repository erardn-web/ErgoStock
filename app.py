...
from utils.gsheets import (
    init_sheets, get_materiel, get_mouvements,
    STATUS_COLORS, TYPES_MOUVEMENT
)
...
if not st.session_state["sheets_ok"]:
    ...
    st.info("👉 Vérifiez votre fichier `.streamlit/secrets.toml` et relancez l'application.")
    st.stop()
...
with left:
    ...
    if sorti.empty:
        ...
        # Enrichir avec la dernière personne emprunteuse
        if not df_mv.empty:
            last_mv = (df_mv
...
if not df_mat.empty and "Catégorie" in df_mat.columns:
    cat_counts = df_mat["Catégorie"].value_counts().reset_index()
...
if not df_mv.empty:
    ...
    if not df_retours.empty:
        try:
            ...
            st.info("Impossible de calculer les retards (dates manquantes).")
    else:
